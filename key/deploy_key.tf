...
resource "null_resource" "private-key-encrypt" {
  ...
  provisioner "local-exec" {
    command = "aws kms encrypt --key-id ${var.kms_key_arn} --plaintext=fileb://base_${var.stack}_${var.git_project}_${var.environment}_${var.region}_id-rsa --output text --query CiphertextBlob | base64  --decode > base_${var.stack}_${var.git_project}_${var.environment}_${var.region}_id-rsa_encrypted"
  }
